...
class RsErrorAnnotator : RsAnnotatorBase(), HighlightRangeExtension {
    ...
    private fun checkUnstableAttribute(ref: RsReferenceElement, o: RsElement, holder: RsAnnotationHolder) {
        ...
        for (outerAttr in element.outerAttrList) {
            val metaItem = outerAttr.metaItem
            if (metaItem.name != "unstable") continue
            val args = metaItem.metaItemArgs ?: continue
            for (innerMetaItem in args.metaItemList) {
                if (innerMetaItem.name != "feature") continue
                val value = innerMetaItem.litExpr?.stringValue ?: continue
                val feature = CompilerFeature(value, FeatureState.ACTIVE)
                feature.check(holder, o, "`${feature.name}`")
            }
        }
    }
